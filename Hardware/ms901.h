 ...
 ****************************************************************************************************
 * @file        atk_ms901m.h
 * @author      正点原子团队(ALIENTEK) / 改编：赛博灵魂 (STM32F10x标准库适配版)
 * @brief       ATK-MS901M (WT901C) 模块协议解析驱动头文件
 ****************************************************************************************************
 */
...
#include "stm32f10x.h"          /* 替换原有的 sys.h，引入标准库核心头文件 */
#include "ms901-uart.h"    /* 引入同级目录下的 UART 底层驱动头文件 */
#include <stddef.h>             /* 引入 NULL 的定义 */
...
typedef enum
{
    ATK_MS901M_LED_STATE_ON  = 0x00,                /* LED灯关闭 (注:原版如此映射) */
